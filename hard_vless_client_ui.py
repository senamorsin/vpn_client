...
def build_connect_command(options: ConnectOptions) -> list[str]:
    ...
    if options.mode in ("system-proxy", "tun-system-proxy"):
...
if PYQT_IMPORT_ERROR is None:
    ...
    class MainWindow(QMainWindow):
        ...
        def _build_config_box(self) -> QWidget:
            ...
            self.mode_box.addItems(["tun", "system-proxy", "tun-system-proxy (recommended)"])
        ...
        def _on_mode_changed(self, mode: str) -> None:
            mode_key = mode.replace(" (recommended)", "")
            tun = mode_key in ("tun", "tun-system-proxy")
            proxy = mode_key in ("system-proxy", "tun-system-proxy")
        ...
        def on_connect(self) -> None:
            opts = ConnectOptions(
                ...
                mode=self.mode_box.currentText().replace(" (recommended)", ""),
