...
{allow_lan_lines if allow_lan_lines else '  # (no LAN CIDRs configured)'}
 ...
 chain output_v6 {{
  ...
  ct state established,related accept
  oifname \"lo\" accept
  oifname \"{cfg.tunnel_iface}\" accept
...
def run_command(cmd: list[str], dry_run: bool, logger: logging.Logger) -> None:
    if dry_run:
        print(shlex.join(cmd))
...
def force_default_route_to_tun(cfg: GuardConfig, dry_run: bool, logger: logging.Logger) -> tuple[list[str], list[str]]:
    ...
    run_command(["ip", "-6", "route", "replace", "default", "dev", cfg.tunnel_iface], dry_run, logger)
...
def restore_default_routes(saved_v4: list[str], saved_v6: list[str], dry_run: bool, logger: logging.Logger) -> None:
    ...
    for line in saved_v6:
        run_command(["ip", "-6", "route", "replace", *line.split()], dry_run, logger)
...
def cmd_disconnect(args: argparse.Namespace) -> int:
    ...
    if args.force_default_route and args.tunnel_iface:
        ...
        run_command(["ip", "-6", "route", "del", "default", "dev", args.tunnel_iface], args.dry_run, logger)
